...
jQuery(document).ready(function() {
    ...
    let status = (hours < 12) ? "Morning" :
        ((hours <= 18 && hours >= 12) ? "Afternoon" : "Evening");

    if (sessionStorage.getItem('storename')) {
        let storename = sessionStorage.getItem('storename');
        let word = camelCase(storename);

        document.querySelector('#storename').textContent = word;
    }
    ...
    $(".utility-menu.dropdown").find('.dropdown-menu').first().stop(true, true).slideUp(150)
        // Register hover listener on anchor tags.
    $('.utility-menu.dropdown').hover(function() {
    ...
    if (jQuery('.subDropdown')[0]) {
        jQuery(document).on("click", ".subDropdown", function() {
